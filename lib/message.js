var db = require('./c_db.js'),
	wid = require('./function/id.js').wid;
...
message.prototype._getCollection_inbox = function() {
	return this.db_get_collection("user.inbox" + this.get("session").uid);
};
...
message.prototype._getCollection_outbox = function() {
	return this.db_get_collection("user.outbox" + this.get("session").uid);
};
 ...
 * @overview: 获取收件箱信息
 */
message.prototype.getInbox = function(option,next) {
	var inbox = this._getCollection_inbox();
	this.db_find.call(inbox,option).toArray(next);
};
