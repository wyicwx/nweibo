 ...
 * @overview: 获取自己的收件箱信息
 */
user.prototype.getSelfInbox = function(next) {
	var message = new Message(this.get("req"));
	var that = this;
	message.getInbox(this.get("session").uid,function(err,data) {
		if(err) throw err;
		if(!data) {
